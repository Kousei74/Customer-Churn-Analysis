...
RAW = str(Path(__file__).resolve().parent.parent / "data" / "raw" / "Telco-Customer-Churn.csv")
...
def normalize_mlflow_uri(user_uri: str | None, default_path: Path) -> str:
    """Return a valid MLflow local file URI for Windows and Unix paths."""
    candidate = default_path.resolve()
    if user_uri is None:
        return candidate.as_uri()
    if "://" in user_uri:
        return user_uri
    return Path(user_uri).expanduser().resolve().as_uri()


def resolve_input_csv(project_root: Path, requested_input: str | None) -> str:
    """Use processed CSV when present; otherwise generate it from raw data."""
    ...
    raw_candidates = []
    if requested_input:
        raw_candidates.append(Path(requested_input).expanduser())
    ...
    if raw_path is None:
        raise FileNotFoundError(
            "No processed CSV found and no raw CSV available in the expected locations."
        )

    print(f"📦 Processed dataset not found. Generating it from raw data: {raw_path}")
    prepare_script = project_root / "scripts" / "prepare_processed_data.py"
    subprocess.run([sys.executable, str(prepare_script)], cwd=str(project_root), check=True)

    if not processed_path.exists():
        raise FileNotFoundError(
            f"Prepared dataset was not created at {processed_path}."
        )
...
def main(args):
    ...
    # Configure MLflow to use a valid local Windows path
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    default_mlflow_path = Path(project_root) / "mlruns"
    mlruns_path = normalize_mlflow_uri(args.mlflow_uri, default_mlflow_path)
    ...
    with mlflow.start_run():
        ...
        print("🔄 Loading data...")
        input_path = resolve_input_csv(Path(project_root), args.input)
        df = load_data(input_path)
        print(f"✅ Data loaded from {input_path}: {df.shape[0]} rows, {df.shape[1]} columns")
...
if __name__ == "__main__":
    ...
    p.add_argument("--input", type=str, default=RAW, help="path to CSV (default: data/raw/Telco-Customer-Churn.csv)")
