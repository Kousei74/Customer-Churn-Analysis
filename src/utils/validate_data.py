...
# ==============================================================================
# OLD VERSION (GX 0.18 / Pre-v1.0 API) - KEPT FOR REFERENCE (COMMENTED OUT)
# ==============================================================================
...
# ==============================================================================
# UPGRADED VERSION (Great Expectations 1.5.8+ / v1.x API)
# ==============================================================================
def validate_telco_data(df) -> Tuple[bool, List[str]]:
    """
    Validate the processed feature matrix used for model training.

    The pipeline loads the processed CSV whenever it exists, so the validation rules
    must match that schema rather than the raw telecom CSV columns.
    """
    print("🔍 Starting data validation with Great Expectations...")

    context = gx.get_context(mode="ephemeral")
    datasource = context.data_sources.add_pandas("pandas_datasource")
    asset = datasource.add_dataframe_asset(name="telco_asset")
    batch_definition = asset.add_batch_definition_whole_dataframe("telco_batch")
    ...
    print("   💼 Validating binary target and feature constraints...")
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(column="Churn", value_set=[0, 1]))

    for column in ["gender", "Partner", "Dependents", "PhoneService"]:
        if column in df.columns:
            suite.add_expectation(
                gx.expectations.ExpectColumnValuesToBeInSet(column=column, value_set=[0, 1])
            )
    ...
    if "MonthlyCharges" in df.columns:
        suite.add_expectation(gx.expectations.ExpectColumnValuesToBeBetween(column="MonthlyCharges", min_value=0, max_value=200))
    ...
    validation_definition = context.validation_definitions.add(
        gx.ValidationDefinition(name="telco_validation", data=batch_definition, suite=suite)
    )
    ...
    for r in results.results:
        if not r.success:
            failed_expectations.append(str(r.expectation_config.kwargs.get("column", r.expectation_config)))
